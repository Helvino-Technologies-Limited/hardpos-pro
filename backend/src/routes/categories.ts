...
router.post('/', requireManager, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await query(
      'INSERT INTO categories (tenant_id, name, slug, description, icon, color, parent_id) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *',
      [req.user!.tenantId, name, slug, description, icon, color, parent_id]
    );
