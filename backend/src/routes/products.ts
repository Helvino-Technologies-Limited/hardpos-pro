...
router.post('/', requireManager, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const {
      ...
      retail_price, trade_price, wholesale_price, cost_price, tax_rate, tax_exempt,
      weight_per_unit, length, width, height, thickness, gauge,
      track_serials, track_batches, allow_fractional, min_quantity,
      reorder_level, max_stock, image_url, tags, attributes,
      is_rentable, rental_daily_rate, rental_deposit
    } = req.body;

    const result = await query(`
      INSERT INTO products (
        tenant_id, name, sku, barcode, description, category_id, unit_id, product_type,
        retail_price, trade_price, wholesale_price, cost_price, tax_rate, tax_exempt,
        weight_per_unit, length, width, height, thickness, gauge,
        track_serials, track_batches, allow_fractional, min_quantity,
        reorder_level, max_stock, image_url, tags, attributes,
    ...
    `, [
      tenantId, name, sku, barcode, description, category_id, unit_id, product_type || 'standard',
      retail_price, trade_price, wholesale_price, cost_price, tax_rate || 16, tax_exempt || false,
      weight_per_unit, length, width, height, thickness, gauge,
      track_serials || false, track_batches || false, allow_fractional || false, min_quantity || 1,
      reorder_level || 0, max_stock, image_url, tags || [], JSON.stringify(attributes || {}),
      is_rentable || false, rental_daily_rate, rental_deposit
    ]);
