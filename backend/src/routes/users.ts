...
router.get('/', requireManager, async (req: AuthRequest, res: Response) => {
  try {
    const result = await query(
      `SELECT id, email, first_name, last_name, role, status, avatar_url, last_login, created_at 
       FROM users WHERE tenant_id = $1 ORDER BY first_name ASC`,
      [req.user!.tenantId]
    );
...
router.post('/', requireAdmin, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await query(`
      ...
      VALUES ($1,$2,$3,$4,$5,$6,$7,'active') RETURNING id, email, first_name, last_name, role, status
    `, [req.user!.tenantId, email, passwordHash, first_name, last_name, phone, role]);
